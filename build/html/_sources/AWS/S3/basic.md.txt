...
## S3内のフォルダの扱い
クラスメソッドの[Amazon S3における「フォルダ」という幻想をぶち壊し、その実体を明らかにする](https://dev.classmethod.jp/articles/amazon-s3-folders/)は読んでおきたい。

ポイントは、S3はkey-valueとしてオブジェクトを管理しており、フォルダという概念はない。
以下の例では、あくまで"foo/"はキーの一部である。
...
|:----|:----|
|foo/hoge.txt|hogehoge|

### 空フォルダについて
GUIやCLIでもS3に対してからのフォルダ"baz/"は作成することができる。
その場合は、ファイルサイズが0の"baz/"というオブジェクトが作成される。
```
(ルート)
 └ baz/
```
...
### S3上でのオブジェクトの管理
空のファイル"baz/"を作成してからその配下にファイル"fuga.txt"を作成した場合と"foo/hoge.txt"を一気に作成した場合、フォルダ構成上は以下となるので同じような構成が2つあるように見える
```
(ルート)
 ├ foo/
 │  ├ hoge.txt
 ...
 │  ├ fuga.txt
```
一方で、AWS上の管理は以下のようになっている。

|キー（フルパス名）|バリュー（ファイルの内容）|
|:----|:----|
|foo/|-|
|foo/hoge.txt|hogehoge|
|baz/fuga.txt|fugafuga|

どのように作成されていたのかに依存して、AWS上の管理が異なる点に注意したい。
また、上記の管理上"fuga.txt"が削除された場合、同時に"baz/"も削除されてしまう。
