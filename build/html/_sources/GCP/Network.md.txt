...
#### Forwarding Zone
特定ドメインのクエリを、指定したDNSサーバーへ転送するゾーン。
...
#### Peering Zone
別VPCのCloud DNSへDNS問い合わせを転送するための仕組み。
...
### DNS Server Policy（通信経路を作るもの）
DNS Server Policy は  
「このVPCがどのようにDNSクエリを送受信するかを定義するポリシー」
...
以下のようなイメージで動作し、動的にルーティングを実施してくれる。
```
[オンプレルータ]
        ↑  BGPで経路交換
        ↓
