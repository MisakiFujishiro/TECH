...
Cloud Routerでは、どの経路を優先して使ってほしいかをBGPの属性を使って制御できる。

その中心になるのが `advertised route priority`。
これは内部的に BGPのMED（Multi-Exit Discriminator）として扱われる。

この優先度は、CloudRouterが広告し、「オンプレミス側が、どの経路でGCPに入るか」を決めるために使われる。
