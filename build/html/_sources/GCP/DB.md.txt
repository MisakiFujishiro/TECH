...
Cloud SQL Auth Proxy を一言で言うと
「IAM 認証を使って Cloud SQL に安全に接続するための中継プロセス」

- Proxy プロセスを起動
- Proxy が サービスアカウントで IAM 認証
- Cloud SQL Admin API を利用してIAM 認証されたセキュアな接続（TLS）を確立
- ローカル（localhost）に TCP ポートを開く
- アプリは localhost:3306 等に接続するだけ

アプリは Cloud SQL の ネットワークや認証方式を意識せず、localhost に接続するだけでよい。
GKEやCompute Engineではよく利用される。
※ Cloud Run では、Cloud Run ではサイドカーを公式にサポートしておらず、単一コンテナ内でプロキシとアプリをまとめる構成はオーバーヘッドや可観測性の点で推奨されません。そのため、公式の Cloud SQL 連携機能が推奨されており、自前で Cloud SQL Auth Proxy を起動するケースは少ない。
...
### レプリケーション
CloudSQLではHA構成のレプリケーションと読み取り専用のReadReplicaだけでなく、一時的にオンプレなどの外部インスタンスを移行用に利用する外部マスター構成のレプリケーション機能が存在する。
...
また、Firestoreはサーバレスであり、自動スケール（読み書きが増えても勝手に伸びる）ため、インスタンス常駐コストないという特徴がある。

### Native/Datastore mode
FirestoreにはNative modeとDatastore modeの2つが存在する。
