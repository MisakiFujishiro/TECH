...
`フィルタ`で、どのログを対象にするかを定め、`宛先`でどのバケットに出力するかを設定する。
...
組織またはフォルダ配下の複数プロジェクトのログのコピーを一括で`エクスポート`するための仕組みである。
...
### ログビュー
Cloud Loggingのログをを誰に見せるかの制御は`ログビュー`で管理される。
...
### メトリクススコープ
メトリクススコープとは、Cloud Monitoringにおいて、複数のプロジェクトのメトリクスをまとめてみるための機能。
メトリクスは、元のPJに残るが、複数のPJのメトリクスやアラートを一つのMonitoringUIで表示可能。
...
## Error Reporting
Cloud Logging などから受け取った情報をもとに  
アプリケーションエラーを抽出・集約し、Google Cloud Console 上で可視化するためのサービス。
エラーを「意味的に同じ単位」でグルーピングし、  人間（運用者・SRE）がエラーの傾向や影響範囲を把握しやすくすることが目的


Error Reportingにエラーを集約する方法は2つある
- Error Reporting API  
    アプリケーションから Error Reporting API の `report` メソッドを呼び出し
- Cloud Loggingから自動抽出  
    Cloud Logging に書き込まれたログのうちError Reporting が認識できる形式・内容を持つログエントリを自動的に検出・集約


ReportedErrorEvent は、Error Reporting が理解できるエラーイベントの正式なデータ構造（JSON スキーマ）である。

- message: UI 上でエラーのタイトルとして表示される
- serviceContext: サービス名、バージョン
- reportLocation: ファイル名、行番号、関数名
- httpRequest: URL、メソッド、レスポンスコードなど

この形式でエラー情報が渡されると、Error Reporting は確実にエラーとして認識・集約できる。