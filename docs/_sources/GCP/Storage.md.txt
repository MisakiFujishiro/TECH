...
CloudStorageで顧客管理鍵を利用する場合、バケットとKMSは同じリージョンにある必要がある点に注意
...
Storage Transfer Service は、ネットワーク経由（オンライン）での大規模データ転送に特化したマネージドサービスである。

主な特徴は以下の通り。

- サードパーティのオブジェクトストレージから Cloud Storage への転送に対応
- 大量データを並列に転送
- 転送失敗時の自動再試行
- 転送後の整合性チェック
- スケジュール実行が可能
...
このサービスは、以下のようなケースを想定して設計されている。

- 数 TB〜数十 TB 程度のデータ量
- ネットワーク帯域が十分に確保できる
- できるだけ早く移行したい
- 人手によるコピーやスクリプト管理を避けたい


#### Transfer Appliance

Transfer Appliance は、物理デバイスを使ったオフラインデータ転送のためのサービスである。

利用の流れは以下の通り。

1. Google から専用のアプライアンス（ストレージ機器）を取り寄せる
2. オンプレミスや既存環境でデータを書き込む
3. アプライアンスを Google に返送する
4. Google が Cloud Storage にデータを取り込む

この方式が想定しているのは、以下のようなケースである。

- 数百 TB〜PB クラスの非常に大きなデータ量
- ネットワーク帯域が制限されている
- オンライン転送に現実的な時間がかかりすぎる
- 物理輸送の方が総時間・総コストで有利


Transfer Applianceによって、暗号化がされているのでCloudStorageに取り込んだ後に、
Transfer Appliance Rehydratorによって、暗号化されたデータを復号する必要がある。