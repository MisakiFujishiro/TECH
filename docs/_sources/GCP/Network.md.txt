...
|種類|制御レベル|判定基準|柔軟性|主な用途|設計思想|向いているケース|
|:----|:----|:----|:----|:----|:----|:----|
|システム生成ルート|自動|宛先IP|低|VPC内部通信|「意識せず通信できる」|サブネット間通信|
|静的ルート（カスタム）|手動|宛先IP|中|固定経路制御|「シンプルに明示的制御」|特定宛先への経路固定|
|ポリシーベースルート（PBR）|手動|宛先 + 送信元|非常に高|トラフィック制御|「意図に基づく制御」|セキュリティ・経路分岐|
|動的ルート（BGP）|自動（学習）|宛先IP|高|ハイブリッド接続|「変化に自動追従」|VPN / Interconnect|
...
`オンプレ → GCP`と`GCP → オンプレ`それぞれの通信について、2本以上経路がある場合に、どのように優先度を決めるかをMEDやadvertised priorityで制御することができる。

オンプレ側とGCP側で設定するものを整理すると以下。

|観点|オンプレ → GCP|GCP → オンプレ|
|:----|:----|:----|
|トラフィック方向|Ingress to GCP|Egress from GCP|
|意思決定者|オンプレ|Cloud Router|
|制御主体|Cloud Router|オンプレ|
|使うもの|advertised priority（= MED）|MED|
|設定場所|GCP側|オンプレ側|
