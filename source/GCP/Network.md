...
VPCネットワークを作成したタイミングでデフォルトとして`デフォルトルート`と`サブネットルート`が自動で登録される。
...
#### ルーティングの種類
VPCで設定されるルートについて、以下のポイントを押さえる

まず、ルートの供給元（Control Plane）

|種類|生成元|自動/手動|概要|主な用途|設計思想|
|---|---|---|---|---|---|
|システム生成ルート|GCP（VPC）|自動|サブネット作成時に自動生成されるルート|VPC内部通信|「何も考えなくても繋がる」|
|静的ルート（カスタム）|ユーザー|手動|宛先CIDRとネクストホップを明示的に定義|固定経路・アプライアンス経由|「明示的に経路を決める」|
|動的ルート（BGP学習）|Cloud Router|自動|オンプレミスや他ネットワークからBGPで学習|VPN / Interconnect|「外部の変化に追従する」|
|（補足）カスタム学習ルート|Cloud Router|手動|BGPで学習したことにするルートを追加|BGP非対応環境の補完|「学習ルートを擬似的に補う」|

次に、ルートの適用方法（Data Plane）

|種類|判定基準|優先度|柔軟性|主な用途|設計思想|
|---|---|---|---|---|---|
|ポリシーベースルート（PBR）|送信元IP + 宛先IP など|最優先|非常に高|トラフィック分岐・セキュリティ制御|「意図に基づくルーティング」|
|通常ルート（最長一致）|宛先IP（CIDR最長一致）|PBRの次|中|一般的なルーティング|「シンプルで予測可能」|

評価フロー
1. PBR が最初に評価される
2. マッチしなければ通常ルート（最長一致）

#### 静的ルートにおけるルーティング先
カスタム静的ルートでは、next hop に指定できるのは：
- VMインスタンス
- インスタンスのNIC
- VPNトンネル
- Cloud NAT（ではない）
...
これは、GCPは「リソースベースルーティング」であり、IP単体ではなく「管理対象のリソース」に送るという思想のため。
...
#### プライベートNAT
プライベートネットワーク同士（オンプレ・他VPC）用の NATをプライベートNATと呼ぶ。

接続元と接続先でIPが重複している場合やルートが多い場合などにプライベートNATを利用してIP変換を行う。

プライベートNATには、HybridNATとNCC用がある
```
Private NAT（大分類）
├─ Hybrid NAT（オンプレ / 他クラウド向け）
└─ Private NAT for NCC spokes（VPC同士向け）
```
...
GCPにおいては、単一のFirewallだけで制御するのではなく、**複数のレイヤ（階層）でFirewallが適用される設計**になっている。

主に以下の3つのレイヤで構成される：

- 階層型ファイアウォールポリシー（組織・フォルダレベル）
- グローバル ネットワーク ファイアウォール ポリシー（Cloud NGFW）
- VPC ファイアウォール ルール（VPC単位）

これらは独立して存在するのではなく、**評価順序に従って順番に適用される**。
階層構造と評価順についてデフォルトだとズレがある点に注意。（以下がデフォルト）

```
[階層型 Firewall]  ← 組織ガバナンス
        ↓
[VPC Firewall] ← 基本的な通信制御
        ↓
[NGFW (Global Network Firewall Policy)] ← 高度セキュリティ（IPSなど）
```
このため、上位レイヤで通信が拒否された場合、下位レイヤには到達しない点に注意が必要。

ただし、network firewall policy enforcement orderによって、順番を一部変更することが可能
- AFTER_CLASSIC_FIREWALL（デフォルト）→先にVPCが評価される
  - Hierarchical
  - `VPC firewall rules`
...
#### 階層型ファイアウォールポリシー（Hierarchical Firewall Policy）
組織（Organization）またはフォルダ単位で適用されるFirewallポリシー。

すべてのプロジェクト・VPCに対して横断的に適用されるため、ガバナンスや統制目的で使用される。
...
ユースケース：
- 外部からのSSH（22番ポート）を全社的に禁止
- 特定のIPレンジからのアクセスを全体で拒否

設計上のポイント：
- 最上位レイヤで評価されるため影響範囲が非常に大きい
- 誤設定すると全環境に影響する

#### グローバル ネットワーク ファイアウォール ポリシー（Cloud NGFW）
Cloud NGFW（Next-Generation Firewall）の機能を提供するファイアウォールポリシー。

従来のIP/ポートベースの制御に加え、L7レベルの検査や脅威検知（IPS）を行うことができる。

主な特徴：
- 複数のVPCにまたがって適用可能
- セキュリティプロファイルを用いた高度な検査
- 通信内容に基づいた制御が可能
...
ユースケース：
- マルウェア通信の遮断
- 不正なアプリケーショントラフィックの検知
- 外部サイトへのアクセス制御（Egress制御）

設計上のポイント：
- VPC Firewallより前に評価される場合がある（設定による）
- ここでブロックされるとVPC Firewallのログは出ない

#### VPC ファイアウォール ルール
VPC単位で定義される、最も基本的なファイアウォール。

L3/L4（IP・ポート）ベースのシンプルな制御を行う。

主な特徴：
- VPCごとに個別に設定
- インスタンス単位（タグ / サービスアカウント）で適用可能
- シンプルで柔軟
...
設計上のポイント：
- 最下位レイヤのため、上位でブロックされると評価されない
- ログはここでマッチした場合のみ出力される
...
#### VPNのルート設計
VPNでのルーティングは、原則として **HA VPN + Cloud Router（BGP）による動的ルーティング** が推奨される。
ただし、オンプレミス機器がBGPに対応していない場合は、Classic VPNによる静的ルーティングを使用する。

|観点|ルートベースVPN|ポリシーベースVPN|
|:----|:----|:----|
|制御主体|ルート|traffic selector|
|トンネルの性質|広い（全部通す）|狭い（限定）|
|CIDR増減|柔軟（ルート追加だけ）|面倒（トンネル or selector変更）|
|設計思想|ネットワーク中心|セキュリティルール中心|
|典型的な用途|HA VPN / 大規模|レガシー機器対応|
...
内部の仕組みとしては、
- VPC内に 「予約されたIPレンジ（allocated range）」 を作る
- そのレンジを Google管理サービス側（別ネットワーク）に委譲する
- 自分のVPCとGoogle管理のVPC(Service Producer Network)に対してVPC Peeringを設定している
...
#### PSAのサブネット
PSAはVPCピアリングをベースとした仕組みであり、L3レベルでネットワーク同士が接続される。
そのため、Cloud SQLも「IPを持つリソース」として扱われる。

このとき、通信を成立させるためには、Google側のサービスが使用するIPアドレス空間が必要になる。
具体的には、VPCから専用のIPアドレス範囲（通常/24以上）割り当てる必要がある。

注意点としては、このIPレンジはユーザのVPC内でサブネットとして使われるわけではない点である。
実際には：
- ユーザがIPレンジを予約する
- Google側がそのレンジを使用する
...
つまり、「自分のVPCのためのサブネット」ではなく、「Googleサービスのために貸し出すIP空間」という位置付けになる。
...
### NEG
NEG（Network Endpoint Group）は：「ロードバランサのバックエンドの実体をどう表現するか」
...
|種類|接続先|主な用途|代表的なユースケース|LBとの関係|
|:----|:----|:----|:----|:----|
|ゾーンNEG（GCE_VM_IP_PORT）|VMのIP:Port|VM単位で細かく制御|Pod単位のルーティング、細かい負荷分散|L7/L4 LB両方|
|インスタンスグループ（参考）|VMグループ|VM単位（まとめて）|従来のスケーリング構成|LBの基本構成|
|サーバーレスNEG|Cloud Run / App Engine / Functions|サーバーレス連携|Cloud RunをLB配下に置く|L7 LBのみ|
|ハイブリッドNEG（オンプレ）|オンプレIP:Port|ハイブリッド構成|オンプレとGCPの統合LB|L7/L4 LB|
|インターネットNEG（外部）|外部FQDN/IP|外部サービス連携|SaaSや外部APIへのLB経由接続|L7 LBのみ|
|PSC NEG|Private Service Connect|Google/他VPCサービス|マネージドサービス接続|L7/L4 LB|
...
#### 広告対象の決め方：デフォルトモード/カスタムモード
CloudRouterがどのプレフィックスをGBPで広告するかを決める設定が、Route Advertisement

Route Advertisementには2つのモードが準備されている

- デフォルト広告モード
  - Cloud Routerが自動的にVPCサブネットを広告
  - Cloud Routerから見えるVPCサブネットを広告する
  - 手動で作成した静的ルートなどは広告されない
- カスタム広告モード
  - 広告するプレフィックスを自分で決めるモード
  - デフォルト広告の自動広告はされない

#### 広告の優先度：MED（advertised route priority）
オンプレ → GCPとGCP → オンプレそれぞれの通信について、2本以上経路がある場合に、どのように優先度を決めるかは方向ごとに異なる仕組みで制御します。
特にGCPでは、MEDそのものではなく `advertised route priority`（GCP独自）を使い、ルートの優先度をオンプレに伝える点が重要。

```
        (priority: 1)   ←優先（低いほど優先）
[オンプレ] ──────→ [Interconnect / VPN A（us-west1）]
        (priority: 1000) ←バックアップ
          └──────→ [Interconnect / VPN B（us-east1）]
```
- advertised route priority：Cloud Routerが広告する際の優先度（低いほど優先）
- MED：標準BGP属性だが、GCPでは直接操作するのではなくadvertised priorityとして扱う
- GCP → オンプレの経路選択は、基本的にオンプレ側のBGPポリシーに依存する（GCP側で強く制御しない）
- プライオリティには、リージョン間コストが加算されるため、十分差をつけておく



#### Regionalモード/Globalモード
...
### Cloud Armorの適用先
Cloud ArmorはLBのBackendを対象に設定するのが主流だが、以下の種類がある。
|ポリシー種類|適用先|特徴|
|:----|:----|:----|
|Backend|LBのバックエンド|L7（HTTP）中心|
|Edge|Google Edge（HTTP(S) LB / CDN）|エッジだがLB前提|
|Network Edge|VMの外部IP|L3/L4で直接保護|
...
### Network Connectivity Center（NCC）
Network Connectivity Center（NCC）は、
Google Cloud における ネットワーク接続の設計・ルーティング制御を集約するコントロールプレーンである。

NCCはVPNやCloud Interconnectのように通信経路そのもの（データプレーン）を提供するサービスではなく、
それらを前提として 複数ネットワーク間のルート共有と接続構成を一元管理する仕組みである。

NCCの役割は「接続すること」ではなく、以下に集約される：

- ルート（経路情報）の集約
- ルートの配布（スポーク間共有）
- ルートの制御（export / import ポリシー）

つまり、
「通信の道を作る」のではなく、「どの道を使うかを全体で統制する」サービスである。
...
```
Hub：ルートの集約・配布を行う中心
Spoke：接続対象（VPC・オンプレ接続など）

重要なのは、
Hubはパケット転送を行わず、ルート共有のみを担う点である。

#### スポークとは
Spokeは「接続対象の種類」によって分類される。

|Spoke種類|内容|役割|
|:----|:----|:----|
|VPCスポーク|VPCネットワーク|ワークロード側|
|ハイブリッドスポーク|VPN / Interconnect|オンプレ接続|
|その他|PSC など|サービス接続|

Spokeはネットワークそのものではなく、
**「そのネットワークをNCCに参加させるための論理的な登録単位」**である。

#### トポロジ設計

NCCでは、スポーク間のルート共有方法を制御することで
複数のトポロジを構成できる。
...
トポロジは物理構成ではなく、以下で制御される：

- export ポリシー（どのルートを出すか）
- import ポリシー（どのルートを受け取るか）

つまり、`トポロジ = ルート配布ポリシーの結果`
...
|IP / 範囲|名前|利用方法|
|:----|:----|:----|:----|
|130.211.0.0/22|Google Front End (GFE) / Load Balancer IP range|外部HTTP(S)ロードバランサからバックエンドへのヘルスチェック・トラフィック許可|
|35.191.0.0/16|同上|同上|
|169.254.169.254|メタデータサーバ|VMからインスタンスメタデータ取得（認証トークン含む）|
|169.254.169.254:53|Cloud DNS（内部）|VPC内の内部DNS解決（メタデータサーバ経由）|
|35.199.192.0/19|Cloud DNS フォワーディング|オンプレからCloud DNSへ転送する際の送信元IP|
|199.36.153.8/30|Private Google Access (PGA)|Google APIへプライベートアクセス（通常）|
|199.36.153.4/30|Restricted Google Access|VPC SC制限付きGoogle APIアクセス|
|34.126.0.0/18|Private Service Connect for Google APIs|PSC経由でGoogle APIにアクセスする場合|
|10.0.0.0/8（割当）|Private Service Access (PSA)|Cloud SQLなどのGoogle管理サービスとVPC接続|
|172.16.0.0/12（割当）|Private Service Access (PSA)|同上（RFC1918範囲から割当）|
|192.168.0.0/16（割当）|Private Service Access (PSA)|同上（RFC1918範囲から割当）|
|8.8.8.8 / 8.8.4.4|Public DNS|Google Public DNS（外部用、GCP内部用途ではない）|
