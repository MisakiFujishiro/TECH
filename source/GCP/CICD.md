...
### CloudBuild プライベートプール
Cloud Buildを利用したCIのパイプラインにおいて、プライベートVPC内のリソースにアクセスしたいという要件に応えるためのサービスがCloud Buildのプライベートプールである。

通常のCloudBuildは、GCP管理する共有ネットワーク上で実行されるため、ユーザーの管理するVPC外で実行されることとなる。

Cloud Build プライベートプールでは、Cloud Build のビルド実行ワーカーを、指定した VPC ネットワーク内で実行する仕組みである。
すなわち、実行プレーン(実態はCompute Engine)をVPC内に起動するイメージ
...
[ ユーザー / Git / API ]
        ...
        v
┌────────────────────────┐
│ Cloud Build Control     │  ← サービス（制御プレーン）
│ Plane (API / Scheduler) │     ※ 常に Google 管理
└────────────────────────┘
        |
        v
┌────────────────────────┐
